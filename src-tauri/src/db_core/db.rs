...
use rusqlite::{ffi, params, Connection, Error as SqlError, OptionalExtension, Result, ToSql};
...
fn map_media_asset_row(row: &rusqlite::Row) -> rusqlite::Result<MediaAsset> {
    Ok(MediaAsset {
        id: row.get(0)?,
...
fn map_media_file_row(row: &rusqlite::Row) -> rusqlite::Result<MediaFile> {
    Ok(MediaFile {
        id: row.get(0)?,
...
fn map_pdf_page_row(row: &rusqlite::Row) -> rusqlite::Result<PdfPage> {
    Ok(PdfPage {
        id: row.get(0)?,
        media_asset_id: row.get(1)?,
...
fn map_catalog_work_row(row: &rusqlite::Row) -> rusqlite::Result<CatalogWork> {
    Ok(CatalogWork {
        id: row.get(0)?,
...
fn map_catalog_work_image_row(row: &rusqlite::Row) -> rusqlite::Result<CatalogWorkImage> {
    Ok(CatalogWorkImage {
        id: row.get(0)?,
        work_id: row.get(1)?,
        image_id: row.get(2)?,
...
fn map_catalog_field_def_row(row: &rusqlite::Row) -> rusqlite::Result<CatalogFieldDef> {
    Ok(CatalogFieldDef {
        id: row.get(0)?,
...
fn map_catalog_preset_row(row: &rusqlite::Row) -> rusqlite::Result<CatalogPreset> {
    Ok(CatalogPreset {
        id: row.get(0)?,
        name: row.get(1)?,
...
fn map_catalog_field_value_row(row: &rusqlite::Row) -> rusqlite::Result<CatalogFieldValue> {
...
fn map_catalog_value_event_row(row: &rusqlite::Row) -> rusqlite::Result<CatalogValueEvent> {
...
impl Database {
    ...
    fn migrate_media_catalog(&self) -> Result<()> {
        let conn = self.conn.lock();
        conn.execute_batch(
            ...
            SELECT
                'ma_' || i.id,
                CASE WHEN i.format = 'pdf' THEN 'pdf' ELSE 'image' END,
                i.id,
                i.sha256_hash,
                i.format,
                i.file_size,
    ...
    fn migrate_catalog_schema(&self) -> Result<()> {
