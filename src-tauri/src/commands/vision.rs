...
pub async fn set_ollama_config(
    ...
    if let Some(u) = url {
        state.db.set_setting("ollama_url", &u).map_err(|e| e.to_string())?;
    }
    if let Some(m) = model {
        state.db.set_setting("ollama_model", &m).map_err(|e| e.to_string())?;
    }
    Ok(())
}
...
pub async fn analyze_images(
    ...
    for (i, image_id) in image_ids.iter().enumerate() {
        ...
        let _ = app.emit("vision-progress", serde_json::json!({
