...
| View menu | Pass | Command Palette, view labels, and shortcuts match the keyboard handler. Current view, sidebar, Preview Display, and web stream state are reflected with checkmarks where applicable. |
...
| Preview Display | `Cmd+Shift+D` | `view_preview_display` | Opens or focuses the dedicated Preview Display window | Pass |
...
- Preview Display is exposed from View with `Cmd+Shift+D`, monitor placement, freeze/blank controls, presets, field-level rail controls, histogram display, and tokenized localhost/LAN web stream lifecycle items.
