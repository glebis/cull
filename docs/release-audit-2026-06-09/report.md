...
- Stage 6 (final, Day-5 + P2): codex APPROVE WITH CHANGES — fixed P1 off-by-one in the token-expiry warn threshold (exactly-7-days now warns; <=, with boundary tests at 7 and 8 days). P2 (structural UI tests) is the established repo source-contract policy (no component-mount infra), runtime-covered by the E2E suite. Permission gating, secret redaction, capability grant, docs-name-accuracy all verified clean.
- Stage 7 (publish-plugin-only feature): codex CLI hung again (frozen CPU) — killed; feature-dev:code-reviewer substitute returned APPROVE WITH CHANGES. One real bug fixed (loader.ts: loadInstalledPlugins full-replaced activePluginIds, wiping bundled cull-publish id when third-party plugins load) via a module-level bundledPluginIds set preserved across both reset points, with regression tests. Minor/intentional findings (staticPublishingEnabled retained as backend gate; pre-existing api.ts {request} wrapper) noted, not blocking. Verdict: APPROVE WITH CHANGES, incorporated.
- Stage 8 (plugins settings tab): codex hung again (frozen CPU, killed) — feature-dev:code-reviewer substitute returned APPROVE WITH CHANGES. Fixed the one spec violation (Registry/Installed groups now gated behind {#if modulePlugins} — toggling had no visible effect before) plus test gaps (Core-no-install assertion, module_plugins visibility gate test, whitespace-query test, join(root) paths, stale comment). Published cull-plugins registry repo description cleaned (dropped Track C3 note — that was the text shown in the UI). Verdict: APPROVE WITH CHANGES, incorporated.
