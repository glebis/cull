<script lang="ts">
    ...
    let { items, width, height, scrollTop, size, gap, cols, focusedIndex, selectedIds, onclick, ondblclick }: Props = $props();
...
<canvas bind:this={canvas} class="overview-canvas" style="top: {scrollTop}px; width: {width}px; height: {height}px;" aria-label="Canvas overview of {items.length} images" tabindex="0" {onclick} {ondblclick}></canvas>

<style>
    .overview-canvas { position: absolute; left: 0; display: block; cursor: crosshair; outline: none; touch-action: pan-y; }
    .overview-canvas:focus-visible { box-shadow: inset 0 0 0 1px var(--blue); }
</style>
