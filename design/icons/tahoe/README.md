...
- `icon-composer-renders/` contains flattened 1024px exports produced by Apple's `ictool` from the `.icon` documents.
- `previews-masked/` mirrors the Icon Composer renders for quick visual review.
- `icns/` contains `.icns` exports created from Icon Composer-rendered iconsets for the Tauri/macOS bundle path.

The runtime alternate icons in `src-tauri/icons/variants/` and the web previews in `static/icon-variants/` are copied from the Icon Composer renders.
