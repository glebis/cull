...
// Guard rail: refuse to tag a release unless the user has produced a
// per-version manual-smoke record and confirmed it in beads. The record
// captures (a) the exact binary SHA they exercised, (b) today's date, and
// (c) the beaded smoke-confirmation close reason. The intent is to make
// "tag and ship" require a human-driven step that can't be silently
// generated by an automated session.
function smokeRecordBlockers(version, releaseCommit) {
  const blockers = [];
  if (!/^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$/.test(version ?? '')) {
    return [`Smoke-record version check rejected malformed version ${JSON.stringify(version)}`];
  }
  if (process.env.CULL_RELEASE_TEST_MODE === '1') {
    if (process.env.CULL_RELEASE_TEST_SMOKE_OK === '1') return [];
    if (process.env.CULL_RELEASE_TEST_SMOKE_MISSING === '1') {
      return [{
        code: 'SMOKE_RECORD_MISSING',
        message: 'No docs/releases/<version>-smoke.md record found for this version',
      }];
    }
    if (process.env.CULL_RELEASE_TEST_SMOKE_STALE === '1') {
      return [{
        code: 'SMOKE_RECORD_STALE',
        message: 'Smoke record binary SHA does not match the current build artifact',
      }];
    }
  }
  const recordPath = join(repoRoot, 'docs', 'releases', `${version}-smoke.md`);
  if (!existsSync(recordPath)) {
    blockers.push({
      code: 'SMOKE_RECORD_MISSING',
      message: `Missing docs/releases/${version}-smoke.md — create the per-version manual-smoke record before tagging`,
      hint: 'run: scripts/install-local-build.sh; then write docs/releases/<v>-smoke.md',
    });
    return blockers;
  }
  const text = readFileSync(recordPath, 'utf8');
  const today = new Date().toISOString().slice(0, 10);
  if (!new RegExp(`^date:\\s*${today}\\b`, 'm').test(text)
    && !new RegExp(`\\b${today}\\b`).test(text)) {
    blockers.push({
      code: 'SMOKE_RECORD_STALE',
      message: `Smoke record date is not today (${today}); refresh after a fresh manual smoke`,
      recordPath: relative(repoRoot, recordPath),
    });
  }
  const bundleBinary = join(repoRoot, 'src-tauri', 'target', 'release', 'bundle', 'macos', 'Cull.app', 'Contents', 'MacOS', 'cull');
  if (existsSync(bundleBinary)) {
    const builtSha = computeFileSha256(bundleBinary);
    const shaMatch = text.match(/binary[_-]?sha256:\s*([0-9a-f]{64})/i);
    if (shaMatch && shaMatch[1] !== builtSha) {
      blockers.push({
        code: 'SMOKE_RECORD_STALE',
        message: 'Smoke record binary SHA does not match the current build artifact',
        recorded: shaMatch[1],
        built: builtSha,
        hint: 're-run scripts/install-local-build.sh and refresh the smoke record',
      });
    }
  }
  // Beads gate: require a closed bead with title matching this version's
  // smoke confirmation. Catches the case where the file exists but no
  // human ever confirmed it.
  try {
    const issues = queryReleaseIncidents();
    const smokeBead = issues.find((issue) => {
      const ref = issue.external_ref ?? issue.externalRef ?? '';
      const title = issue.title ?? '';
      const inText = `${ref} ${title}`;
      return (ref === `cull-release-${version}-smoke` || title.includes(`smoke ${version}`))
        && inText.includes(version);
    });
    if (!smokeBead) {
      blockers.push({
        code: 'SMOKE_BEAD_MISSING',
        message: `No smoke-confirmation bead for ${version} found; create one with external_ref cull-release-${version}-smoke and close it after manual testing`,
      });
    } else if (smokeBead.status !== 'closed') {
      blockers.push({
        code: 'SMOKE_BEAD_OPEN',
        message: `Smoke-confirmation bead ${smokeBead.id} for ${version} is not closed (status=${smokeBead.status})`,
      });
    }
  } catch (error) {
    blockers.push(`Smoke-bead lookup failed: ${error.message ?? String(error)}`);
  }
  return blockers;
}
...
function runPrepare(args) {
  ...
  return {
    ...
    state: record.state,
    releaseCommit,
    files: plan.edits.map((edit) => edit.path),
    nextSteps: [
      `install + smoke: scripts/install-local-build.sh; open /Applications/Cull.app`,
      `write docs/releases/${plan.version}-smoke.md (date, binary-sha256, manual checklist)`,
      `create + close bead with external_ref cull-release-${plan.version}-smoke`,
      `then: npm run release:cull -- tag --version ${plan.version} --expected-source ${releaseCommit}`,
    ],
  };
}
...
function runTag(args) {
  ...
  const readinessBlockers = [
    ...homebrewCaskBlockers(config),
    ...releaseIncidentBlockers(config),
    ...smokeRecordBlockers(record.version, record.releaseCommit),
  ];
